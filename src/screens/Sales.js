...
import { getStores, postsalesOrders } from "../api";
import PageSwitcher from "../components/PageSwitcher";
import SalesOrderTable from "../containers/SalesOrderTable";
function Sales(props) {
  ...
  const [SalesInvoiceFailed, setSalesInvoiceFailed] = useState(false);
  ...
  const getReservationList = async () => {
    ...
    let payload = {
      ...
      sales_invoice_failed: SalesInvoiceFailed === "true" ? true : false,
      searchTerm: searchFilter,
      store_id: selectedSite,
    };
    const response = await postsalesOrders(payload, startLimit, endLimit);
    ...
    <Flex direction="column">
      ...
      <Flex width={"100%"} wrap="wrap" justifyContent="space-between" my="10px">
        ...
        <Select
          ...
          value={SalesInvoiceFailed}
          onChange={(e) => {
            setPageNumber(1);
            setSalesInvoiceFailed(e?.target?.value);
          }}
          size="sm"
          width="150px"
          borderRadius="5px"
